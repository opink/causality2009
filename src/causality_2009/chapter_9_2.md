...
&emsp;&emsp;式(9.9)根据式(9.3)和式(9.10)，使用 \\(A=y_x , B=y'\_{x'} , P(y_x)=P(y|x) 和 P(y'\_{x'})=P(y'|x') \\) 得到。
...
&emsp;&emsp;单调性表示这种假设：在任何情况下，从 \\( X = false 到 X = true\\) 的变化都不能使 *Y* 从 true 变为 false。在流行病学中，这一假设常常被表达为"无预防"，也就是说，人群中人和个体都不能通过暴露危险而避免疾病。
...
&emsp;&emsp;定理9.2.10~定理9.2.14中建立的关系时基于外生性假设的。在本节中，我们放宽了这一假设，并考虑 *X* 对 *Y* 的影响有混杂的情况，即 \\( P(y_x) \neq P(y|x) \\)。在这种情况下，\\( P(y_x)\\) 仍然可以通过辅助手段(例如，通过校正某些协变量或通过实验研究)来估计，但问题是，这些附加信息是否能够使英国关系的概率变得可识别。答案是肯定的：
