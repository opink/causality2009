...
```admonish note
"君子生非异也善假于物也":智能体的本质优势在于发现并利用问题的不变性。

不论是(人经历过事后会有的一点geometric intuition人人都会有的通过置信度局部引导)，还是(精确的代数操作)，还是(RL旨在学习最大化预期奖励的搜索训练)，以及(受控试验的扩张)，都脱不开 问题不变性的保证。

—————以上是存在于过程中的事物—————

以下是过程本身： 
  
  负熵流是这一切产生的暗金线，能源是负熵流的一种载体，现行集中化石能源负熵流生成的集约生产和社会运送负熵流的组织，是这样的资本主义形式，占据有利位置的但“打不过”的，那肤浅智慧那就是逃税、避税、减低被抽税的流转次数，流动性静止到股市中/18次危机4次战争里通过危机廉价收割负熵流。

  理想主义的理想的目的是分布式可获得负熵流，只有在此种能流形式下，才会有不同的社会组织形式物理化，“打不过先加入”也可以是现行的组织生产的手段，但如何解放占据有利位置的具体导向却是不同了，此才为实践的智慧。

```

## “不假于外“是种心态，是用来控制情绪的，没别的，别延伸

```admonish note
曾经我从内心做过多次的计划“对没踏足的领域的外文材料直接学，怕事倍功半，希望事半功倍，又怕功倍事也不成”。

但是回看，在细品，通过近似动态规划和贯序决策以及因果推断的学习，搞明白了，对简单的问题是可以规划的，但是现实中的混杂复杂问题，就不是仅仅靠脑子内的计划能解决的了，这是物质基础决定的，需要通过do算子来解决MDP，实践来验证和调整。

直接跳出心神和情绪，需要科学的假于外物了。
```
